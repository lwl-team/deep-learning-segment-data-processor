...
def my_rename(path):
    ...
    k = 1
    for i in a_l:
        new_i = str(k) + '.' + i.split('.')[1]
        os.rename(a + '/' + i, a + '/' + new_i)
        k += 1
    k = 1
    for i in b_l:
        new_i = str(k) + '.' + i.split('.')[1]
        os.rename(b + '/' + i, b + '/' + new_i)
        k += 1
...
def pre_main(img):
    ...
    if IMG_FORMAT=='png':
        ROTATE_TYPE_LIST=[Image.FLIP_LEFT_RIGHT,Image.FLIP_TOP_BOTTOM,Image.ROTATE_90,Image.ROTATE_180,Image.ROTATE_270]
